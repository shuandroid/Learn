...
import com.chendroid.learn.util.ViewUtils
    ...
    override fun onInterceptTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                Log.i(
                    "zc_test",
                    "mTouchDownX is $mTouchDownX, and x is ${event.x}, and mTouchDownY is $mTouchDownY, and y is ${event.y}"
                )
                Log.i(
                    "zc_test",
                    "scaledTouchSlop is ${ViewConfiguration.get(context).scaledTouchSlop}, and delta is $delta"
                )
                mScrolling = delta >= ViewConfiguration.get(context).scaledTouchSlop

                Log.i(
                    "zc_test",
                    "TestConstraintLayout onInterceptTouchEvent ACTION_MOVE mScrolling is $mScrolling"
                )
            }
...
//                Log.i("zc_test", "onTouchEvent() ACTION_MOVE ")
    ...
    fun setTargetView(view: View) {
        targetDragView = view
        targetDragView.post {
            ViewUtils.fetchBitmapFromView(
                targetDragView,
                targetDragView.context as Activity
            ) { bitmap ->
    ...
    private fun handleDragMoving() {
        ...
        Log.i(
            "zc_test",
            "handleDragMoving 这里 mCurX is $mCurX, mPrevX  is $mPrevX， mCurY is $mCurY，mPrevY is $mCurY  "
        )
